...
export default function SickCityPatientEquipment({patientRef,equipment,procedural=false}: {patientRef:RefObject<THREE.Group|null>;equipment:WorldCareEquipment;procedural?:boolean}) {
  if(!equipment.bagOpen) return null;
  return <>
    {equipment.oxygenApplied && <>
      <group position={[1.8,.32,.65]}>
        ...
        <Html center position={[0,.6,0]} zIndexRange={[40,30]} style={{pointerEvents:'none'}}><span aria-label="Oxygen support active" style={{color:'#b7eadc',background:'#102620',borderRadius:8,padding:'4px 8px',fontSize:11,whiteSpace:'nowrap'}}>Oxygen applied</span></Html>
      </group>
    ...
    <group position={[1.35,.07,.65]}>
      ...
      {(equipment.bloodPressure || equipment.spo2 !== undefined) && <Html center position={[0,.3,0]} zIndexRange={[40,30]} style={{pointerEvents:'none'}} calculatePosition={(object,camera,size)=>{
        const point=new THREE.Vector3().setFromMatrixPosition(object.matrixWorld).project(camera);
        return [Math.max(115,Math.min(size.width-115,(point.x+1)*size.width/2)),Math.max(50,Math.min(size.height-50,(1-point.y)*size.height/2+80))];
      }}>
        <div aria-label="Patient equipment readings" style={{background:'rgba(9,20,27,.92)',border:'1px solid #4b807f',borderRadius:9,padding:'8px 12px',color:'#a9f0df',fontFamily:'monospace',fontSize:12,whiteSpace:'nowrap'}}>
