...
### Teen breathing response pilot

The SickCity teen call now feeds its recorded respiratory rate into the rig's chest animation, with smooth transitions and reduced effort only after treatment plus completed reassessment. Selecting medication does not imply recovery. A compact monitor observation distinguishes treatment given from confirmed reassessment and uses the clinical engine's existing breathing, responsiveness, and measured vital signs. Equipment and response state survive the transition to stretcher care and hospital receiving; the driving objective reports the last reassessment rather than claiming a live simulation. Existing per-call cleanup resets them. This pilot does not add deterioration rules, voice acting, facial animation, or new treatment guidance.

### Responses across clinical calls

Hypoglycemia, opioid overdose, chest pain, and the crash call now publish their own recorded breathing and responsiveness alongside vital signs. Treatment and reassessment remain separate stages; the required interventions match each existing scenario. Crash reassessment keeps shallow breathing, records the faster respiratory rate, and explicitly flags worsening rather than applying the teen's reduced-effort animation. Opioid breathing uses the scenario's recorded rate and shallow initial motion. The animated lying patient only settles after completed reassessment, not immediately after medication. Quick calls retain their existing behavior. Unit coverage checks every clinical response, and the full clinical transport regression checks one response display on the cot, the last-reassessment report while driving, and cleanup after delivery.
